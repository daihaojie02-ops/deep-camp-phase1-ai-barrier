...
**我做了什么：** 阅读 Task 002 的任务描述（11 项必提交内容、排除范围、评分标准），确认 Topic A（信息检索→深度报告）不碰数据分析/Agent搭建/工程化排除红线。
...
**我做了什么：** 逐节阅读 AI 生成的报告初稿，删除了所有"总之""综上所述"开头的冗余总结段，在每个框架分析末尾用自己的话写了"我的判断"段落（基于我的自评水平、学习时间、Task 3 计划）。
...
3. **有意识的边界划定。** 信息检索场景 ≠ Agent 搭建。不为了"展示工具使用能力"强行跑 4 个框架的 Demo——那会占用 Task 3 的范围且超出截止时间
